...
- Timeline:
  - New event `Call Node`: Call a Godot NodePath and a method name. In addition you can add arguments as well. The Timeline will execute those methods and wait for completion, if the method in question is async and/or yielding.
  - You now can drag and drop events into the timeline! 
- Dialog node
  - Fixed a bug where Glossary Definitions plain text was being added to the name label.
- Thanks [ellogwen](https://github.com/ellogwen) for your contributions on the new `Call Node` and the drag and drop events!

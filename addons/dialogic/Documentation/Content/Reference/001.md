...
### change_timeline()
`change_timeline(timeline)` is used to change the current timeline. If a Dialog Node is in a scene, it will immediately play the new timeline's first event.
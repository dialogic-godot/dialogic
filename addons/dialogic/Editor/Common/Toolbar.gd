...
## Script that handles base toolbar management. Also check out EditorsManager script.  

enum ButtonLocations {Left, Center, Right}
...
func add_button(label:String, icon:Texture, location:= ButtonLocations.Left) -> void:
	var button: Button = Button.new()
	button.text = label
	button.icon = icon
	match location:
		ButtonLocations.Left:
			$LeftButtons.add_child(button)
		ButtonLocations.Center:
			$CenterButtons.add_child(button)
		ButtonLocations.Right:
			$RightButtons.add_child(button)
	return button

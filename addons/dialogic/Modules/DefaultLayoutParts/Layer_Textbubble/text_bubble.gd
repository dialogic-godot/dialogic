...
@onready var choice_container: Container = ($DialogText/ChoiceContainer as Container)
...
func _process(delta:float) -> void:
	if speaker_node:
		if speaker_node is Node3D:
			base_position = get_viewport().get_camera_3d().unproject_position((speaker_node as Node3D).global_position)
...
func add_choice_container(node:Control, alignment:=FlowContainer.ALIGNMENT_BEGIN) -> void:
	text.add_child(node)

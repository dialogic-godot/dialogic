...
func _connect_all_buttons():
	for child in get_parent().get_children():
		if child is DialogicDisplay_ChoiceButton:
			child.button_up.connect(_on_pressed, [child.sound_pressed])
			child.mouse_entered.connect(_on_hover, [child.sound_hover])
			child.focus_entered.connect(_on_focus, [child.sound_focus])

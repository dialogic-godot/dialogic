...
func _copy_signals(target:Node):
	var sigs = self.get_signal_list()
	for s in sigs:
		if not target.has_signal(s['name']):
			print("Cannot copy connections of signal " + s['name'] + " from " + self.to_string() + " to " + target.to_string())
			continue
		var conns = self.get_signal_connection_list(s['name'])
		for c in conns:
			target.connect(c['signal'], target, c['method'], c['binds'], c['flags'])

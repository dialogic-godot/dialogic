...
const DEFAULT_THEME_PATH = 'res://addons/dialogic/Editor/ThemeEditor/default-theme.cfg'
const DEFAULT_AUDIO_FILE = "res://addons/dialogic/Example Assets/Sound Effects/Beep.wav"
const DEFAULT_BOX_SIZE = Vector2(910, 167)
...
enum STATE {
	...
	READY,			# Text already displayed
	TYPING,			# While typing text
	WAITING,		# Waiting for an internal event (like a timer)
	WAITING_INPUT,	# Waiting for player input
	ANIMATING		# Performing an animation
}
var _state: int = STATE.IDLE

var do_fade_in: bool = true
var dialog_faded_in_already: bool = false
# True if the glossary popup is visible: # TODO rename
var definition_visible: bool = false setget set_definition_visible, get_definition_visible
...
onready var DefinitionInfo = $DefinitionInfo
onready var CharacterVoice = $FX/CharacterVoice
...
func set_definition_visible(new_value: bool):
	DefinitionInfo.visible = definition_visible


func get_definition_visible() -> bool:
	return DefinitionInfo.visible
...
func _ready():
	...
	# Check if the dialog should be read the code from a external file
	...
	_sync_mouse_filter_with_settings()
...
func _sync_mouse_filter_with_settings():
...
func load_config_files():
	...
	# Theme
	var theme_file = DEFAULT_THEME_PATH
	...
	if settings.has_section('history'):
		_load_history_from_settings(theme_file)


func _load_history_from_settings(theme_file: String):
...
## 						VISUALS
## -----------------------------------------------------------------------------
# Makes sure that the dialog is displayed at the correct size and position.
func resize_main():
...
func load_theme(filename):
	...
	# Box size
	call_deferred('deferred_resize', TextBubble.rect_size, theme.get_value('box', 'size', DEFAULT_BOX_SIZE), current_theme_anchor)
...
## -----------------------------------------------------------------------------
# Start dialog parsing from a timeline path
func set_current_dialog(timeline_path: String):
	current_timeline = timeline_path
	dialog_script = DialogicResources.get_timeline_json(timeline_path)
	return load_dialog()

# Starts dialog parsing
func load_dialog():
...
func _process(delta):
	...
	TextBubble.next_indicator_visible = is_state(STATE.READY)
	...
	if current_event.has('text'):
		if '[nw]' in current_event['text'] or '[nw=' in current_event['text']:
			TextBubble.next_indicator_visible = false
		...
		if is_state(STATE.WAITING):
			...
			if is_state(STATE.TYPING):
				...
				if current_event.has('options') and !is_state(STATE.WAITING_INPUT):
					...
					pass
				elif TextBubble.next_indicator_visible:
					CharacterVoice.stop_voice() # Stop the current voice as well
					play_audio("passing")
					_load_next_event()
				else:
					next_event(false)
...
# When the text finished showing plays audio, adds buttons, handles [nw]
func _on_text_completed():
	...
	if current_event.has('options'):
		_create_options(current_event['options'])
		...
		if '[nw]' in current_event['text'] or '[nw=' in current_event['text']:
			...
			if '[nw=' in current_event['text']: # Regex stuff
				...
				if(waiting_time.begins_with('v')):
					waiting_time = CharacterVoice.remaining_time()
				else:
...
func _create_options(options):
	...
	for option in options:
		if _should_add_choice_button(option):
			add_choice_button(option)
...
# Checks and calls the above functions
...
func _handle_text_event(event: Dictionary):
...
func _handle_character_event(event: Dictionary):
...
func _handle_question_event(event: Dictionary):
...
func _handle_condition_event(event: Dictionary):
...
func _handle_end_branch_event(event: Dictionary):
	emit_signal("event_start", "endbranch", event)
	_load_next_event()


func _handle_set_value_event(event: Dictionary):
...
func _handle_anchor_event(event: Dictionary):
	emit_signal("event_start", "anchor", event)
	_load_next_event()


func _handle_goto_event(event: Dictionary):
	emit_signal("event_start", "goto", event)
	dialog_index = anchors[event.get('anchor_id')]
	_load_next_event()


func _handle_change_timeline_event(event: Dictionary):
...
func _handle_change_background_event(event: Dictionary):
...
func _handle_close_dialog_event(event: Dictionary):
...
func _handle_wait_seconds_event(event: Dictionary):
...
func _handle_set_theme_event(event: Dictionary):
...
func _handle_set_glossary_event(event: Dictionary):
...
func _handle_save_event(event: Dictionary):
...
func _handle_audio_event(event: Dictionary):
...
func _handle_background_music_event(event: Dictionary):
...
func _handle_emit_signal_event(event: Dictionary):
	emit_signal("dialogic_signal", event['emit_signal'])
	_load_next_event()


func _handle_change_scene_event(event: Dictionary):
	if event.has('scene'):
...
func _handle_call_node_event(event: Dictionary):
...
func event_handler(event: Dictionary):
	...
	match event['event_id']:
		# MAIN EVENTS
		'dialogic_001':
			_handle_text_event(event)
		'dialogic_002':
			_handle_character_event(event)
		
		# LOGIC EVENTS
		'dialogic_010':
			_handle_question_event(event)
		'dialogic_011':
			_handle_choice_event(event)
		'dialogic_012':
			_handle_condition_event(event)
		'dialogic_013':
			_handle_end_branch_event(event)
		'dialogic_014':
			_handle_set_value_event(event)
		'dialogic_015':
			_handle_anchor_event(event)
		'dialogic_016':
			_handle_goto_event(event)
		
		# TIMELINE EVENTS
		'dialogic_020':
			_handle_change_timeline_event(event)
		'dialogic_021':
			_handle_change_background_event(event)
		'dialogic_022':
			_handle_close_dialog_event(event)
		'dialogic_023':
			_handle_wait_seconds_event(event)
		'dialogic_024':
			_handle_set_theme_event(event)
		'dialogic_025':
			_handle_set_glossary_event(event)
		'dialogic_026':
			_handle_save_event(event)
		
		# AUDIO EVENTS
		'dialogic_030':
			_handle_audio_event(event)
		'dialogic_031':
			_handle_background_music_event(event)
		
		# GODOT EVENTS
		'dialogic_040':
			_handle_emit_signal_event(event)
		'dialogic_041':
			_handle_change_scene_event(event)
		'dialogic_042':
			_handle_call_node_event(event)
		_:
...
# Shows the given text in the Text Bubble, handles the simple translation feature
...
func add_choice_button(option: Dictionary) -> Button:
	...
	# otherwise default to hotkeys 1-9 for the first 9 buttons
	...
	if Input.get_mouse_mode() != Input.MOUSE_MODE_VISIBLE:
		...
		# Make sure the cursor is visible for the options selection
		Input.set_mouse_mode(Input.MOUSE_MODE_VISIBLE)
	
	return button

# Evaluates given option's condition
func _should_add_choice_button(option: Dictionary) -> bool:
...
# Instances custom choice buttom
...
# Instances normal dialogic button
func get_classic_choice_button(label: String) -> Button:
	var theme = current_theme
...
# Connects signals after the short delay to make accidental clicking less likely
...
func handle_voice(event):
	...
	if event.has('voice_data'):
		...
		if voice_data.has('0'):
			CharacterVoice.play_voice(voice_data['0'])
			return
...
## 						PORTRAITS
## -----------------------------------------------------------------------------
# Unfocus all characters except argument
...
# Moves all portraits out of view
...
# Calculates where to move the portrait to, so the fake-z-index looks good 
...
func _on_RichTextLabel_meta_hover_started(meta):
	_show_glossary_popup(meta)


func _show_glossary_popup(id):
...
func _on_RichTextLabel_meta_hover_ended(meta):
	_hide_glossary_popup()


func _hide_glossary_popup():
	# Adding a timer to avoid a graphical glitch
...
# Loads all persistent data
func resume_state_from_info(state_info: Dictionary):
	do_fade_in = false
	yield(self, "ready") # Wait until node is in tree
	
	# Load characters
	for saved_portrait in state_info['portraits']:
		var event = saved_portrait
		
		# TODO: this code is ALL copied from the event_handler. So I should probably outsource it to a function...
		var character_data = DialogicUtil.get_character(event['character'])
		if portrait_exists(character_data):
			for portrait in $Portraits.get_children():
			...
			p.move_to_position(get_character_position(event['position']))
			# This info is used to save the state later
			p.current_state['character'] = event['character']
			p.current_state['position'] = event['position']

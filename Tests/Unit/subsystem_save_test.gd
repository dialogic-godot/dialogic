...
const EXAMPLE_SEEN_HISTORY: Dictionary = { "res://Dialogic/Timelines/start.dtl1": 1,
...
func test_save_already_seen() -> void:
    ...
    var global_data_seen_events: Dictionary = save.get_global_info("already_seen_history", {})
...
func test_deletion_save_already_seen() -> void:
    ...
    assert(save.get_global_info("already_seen_history", {}) == {}, "Global data should be empty after reset.")
